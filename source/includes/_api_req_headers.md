...
| --- | --- | --- |
| `If-Modified-Since` | Uses an <a href="http://tools.ietf.org/html/rfc2822#section-3.3" target="_blank">RFC 2822</a> date. If supplied, then only resources modified since the specified date will be returned. If there are no modified objects, then a `304 Not Modified` response will be sent. Please refer to the individual resource pages for support for this header. | Use `min_date_modified` and `max_date_modified` query parameters on resources that support them. |
